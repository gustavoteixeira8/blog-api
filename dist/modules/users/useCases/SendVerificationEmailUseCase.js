...
let SendVerificationEmailUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const expiresDate = this._dateProvider.add(new Date(), {
      minutes: 30
    });
    ...
    await Promise.all([this._userTokenRepository.save(newUserToken), this._mailQueueProvider.add({
      ...
      subject: `Email verification - ${_app.appConfig.name}`,
      context: {
        user: {
